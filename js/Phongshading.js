...
// 自定义光源颜色
var customLightColor = vec3(1.0, 1.0, 1.0); // 默认为白色
...
window.onload = function() {
	...
	gl.clearColor(0.85, 0.95, 0.95, 1.0); //设置背景色 
...
function uploadTexture(file) {
    ...
    reader.onload = function(e) {
        ...
        image.onload = function() {
            // 更新方块纹理
            cubeTexture = configureTexture(image);
            render();
        };
...
function render(){	
    ...
    if(lightType === 1.0) { // 点光源
	    lightProjection = perspective(120.0, 1.0, 0.1, lightRadius + 50.0);
	} else { // 平行光
	    lightProjection = ortho(-10.0, 10.0, -10.0, 10.0, 0.1, 50.0);
	}
	...
	// 传递光源颜色（使用自定义颜色）
	gl.uniform3fv( gl.getUniformLocation( program, "lightColor" ), flatten(customLightColor));
	
	// 传递雾化参数
	gl.uniform1i( gl.getUniformLocation(program, "fogEnabled"), fogEnabled ? 1 : 0);
    ...
    // 更新灯光的颜色
    gl.uniform3fv( gl.getUniformLocation( lampPorgram, "lightColor" ), flatten(customLightColor));
    ...
    // 为天空盒传递雾化参数
    gl.uniform1i( gl.getUniformLocation(skyboxProgram, "fogEnabled"), fogEnabled ? 1 : 0);
    gl.uniform1f( gl.getUniformLocation(skyboxProgram, "fogDensity"), fogDensity);
    gl.uniform3fv( gl.getUniformLocation(skyboxProgram, "fogColor"), flatten(fogColor));
    gl.uniform3fv( gl.getUniformLocation(skyboxProgram, "viewPos"), flatten(eyePos));
...
window.onkeydown = function(e){
    ...
    switch (code) {
		...
		case 70: // F键 - 切换雾化效果开关
			...
			console.log("雾化效果: " + (fogEnabled ? "开启" : "关闭"));
			break;
		case 71: // G键 - 增加雾化浓度
			fogDensity += 0.005;
			if (fogDensity > 0.2) fogDensity = 0.2;
			console.log("雾化浓度: " + fogDensity.toFixed(3));
			break;
		case 72: // H键 - 减少雾化浓度
			fogDensity -= 0.005;
